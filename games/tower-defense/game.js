...
function showToast(message, tone = "info") {
  ...
  ui.toast.className = `toast toast-${tone} toast-visible`;
...
function addFloatingText(x, y, text, color = "#f5f7fb") {
  floatingTexts.push({ x, y, text, color, life: 1, vy: -42 });
}
...
const levels = [
  {
    ...
    theme: {
      ...
      style: "industrial",
      image: "assets/levels/level1.jpg",
    },
    path: [
      { x: -40, y: 320 },
      { x: 180, y: 320 },
...
enemyImage.onerror = () => {
  setMenuMessage("Nan enemy image missing. Check assets/nan/enemy.png.");
};
trackImage(enemyImage);
...
const towerImageSources = {
  gun: "assets/towers/gun1.jpg",
  cannon: "assets/towers/gun2.jpg",
  sniper: "assets/towers/gun3.jpg",
  cryo: "assets/towers/gun4.jpg",
};
const towerImages = {};
...
const unlockKey = "nan-arcade-td-unlocks";
const langKey = "nan-arcade-td-lang";
let unlockedLevels = [];
let currentLang = "en";
...
function loadUnlocks() {
  const saved = localStorage.getItem(unlockKey) || localStorage.getItem("face-td-unlocks");
  if (saved) {
    try {
...
function loadLanguage() {
  const saved = localStorage.getItem(langKey) || localStorage.getItem("face-td-lang");
...
function updateWaveButton() {
  if (!ui.startWaveBtn) return;
  const ready = gameState === "playing" && !waveInProgress && wave < wavesTarget() && lives > 0;
  ui.startWaveBtn.classList.toggle("pulse-ready", ready);
  ui.startWaveBtn.disabled = !ready && gameState === "playing";
}
...
function startWave() {
  ...
  showToast(i18n[currentLang].hintWave(wave), "wave");
  waves.push(waveData);
  playSound("wave");
  updateUI();
  updateWaveButton();
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (dist < 2) {
      ...
      if (enemy.waypoint >= pathPoints.length) {
        ...
        lives -= 1;
        basePulse = 1;
        triggerShake(enemy.kind === "boss" || enemy.kind === "zac" ? 12 : 7);
        showToast(i18n[currentLang].hintSlip, "danger");
        ...
        if (lives <= 0) {
          ...
          showToast(i18n[currentLang].hintGameOver, "danger");
          setGameState("menu");
        }
      }
      continue;
    ...
    enemy.x += vx;
    enemy.y += vy;
    enemy.bob = Math.sin(gameTime * 6 + enemy.x * 0.02) * 2.5;
  }
}
...
function applyDamage(enemy, amount) {
  enemy.hp -= amount;
  enemy.hitFlash = 0.18;
  addFloatingText(enemy.x, enemy.y - enemy.radius, `-${Math.max(1, Math.round(amount))}`, "#ffb4a2");
  playSound("hit");
  if (enemy.hp <= 0) {
    money += enemy.bounty;
    ...
    addFloatingText(enemy.x, enemy.y - enemy.radius - 10, `+$${enemy.bounty}`, "#7dffb2");
...
function updateSpawn(dt) {
  ...
  if (enemiesToSpawn === 0 && enemies.length === 0) {
    ...
    if (wave >= wavesTarget()) {
      ...
      showToast(i18n[currentLang].hintWaveClear, "success");
    }
  }
}
...
function drawPath() {
  ctx.save();
  ctx.shadowColor = "rgba(245, 177, 46, 0.35)";
  ctx.shadowBlur = 18;
...
function drawBase() {
  ...
  if (!end) return;
  const pulse = 1 + Math.sin(gameTime * 4) * 0.06;
  const flash = basePulse > 0 ? 1 + basePulse * 0.4 : 1;
  ctx.save();
  ctx.translate(end.x, end.y);
  ctx.fillStyle = `rgba(255, 90, 70, ${0.18 * flash})`;
  ctx.beginPath();
  ctx.arc(0, 0, 34 * pulse * flash, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = `rgba(255, 180, 120, ${0.7 * flash})`;
  ctx.lineWidth = 2;
  ctx.beginPath();
...
function drawTowers() {
  ...
  for (const tower of towers) {
    ...
    if (tower === selectedPlacedTower) {
      const pulse = 0.85 + Math.sin(gameTime * 5) * 0.15;
      ctx.strokeStyle = `${type.color}88`;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(tower.x, tower.y, type.range * pulse, 0, Math.PI * 2);
      ctx.stroke();
      ctx.strokeStyle = type.color;
      ctx.lineWidth = 1;
      ctx.setLineDash([6, 8]);
      ctx.lineDashOffset = -pathDashOffset * 0.5;
      ctx.beginPath();
...
function drawEnemies() {
  for (const enemy of enemies) {
    const bob = enemy.bob || 0;
    ctx.save();
    ctx.translate(enemy.x, enemy.y + bob);
    const size = enemy.radius * 2.64;
    const img = enemy.kind === "zac" ? zacImage : enemyImage;
    if (enemy.hitFlash > 0) {
      ctx.globalAlpha = 0.65 + Math.sin(gameTime * 40) * 0.2;
    }
    if (enemy.slowFactor && enemy.slowFactor < 1) {
      ctx.shadowColor = "rgba(125, 220, 255, 0.8)";
      ctx.shadowBlur = 14;
    }
    if (img.complete && img.naturalWidth > 0) {
    ...
    if (enemy.hitFlash > 0) {
      ...
      ctx.fillStyle = `rgba(255, 80, 80, ${enemy.hitFlash * 2})`;
      ctx.fillRect(-size / 2, -size / 2, size, size);
      ctx.globalCompositeOperation = "source-over";
    }
    ctx.restore();

    const barWidth = Math.max(40, enemy.radius * 1.4);
    const healthRatio = enemy.hp / enemy.maxHp;
    ctx.fillStyle = "rgba(0,0,0,0.55)";
    ctx.fillRect(enemy.x - barWidth / 2, enemy.y - enemy.radius - 16 + bob, barWidth, 6);
    ctx.fillStyle = enemy.kind === "boss" || enemy.kind === "zac" ? "#f59e0b" : "#76ff84";
    ctx.fillRect(enemy.x - barWidth / 2, enemy.y - enemy.radius - 16 + bob, barWidth * healthRatio, 6);
  }
}
...
function update(timestamp) {
  ...
  lastTime = timestamp;
  gameTime += dt;
  pathDashOffset += dt * 40;
  if (basePulse > 0) basePulse = Math.max(0, basePulse - dt * 2.5);
...
applyLanguage();
loadTowerImages();
if (!localStorage.getItem(langKey) && !localStorage.getItem("face-td-lang")) {
